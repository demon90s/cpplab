...
void test_strncpy2()
{
	...
	strncpy(buffer, "wow", 0);	// nothing will be copied

	puts(buffer);
}

void safe_strncpy(char *des, const char *src, size_t des_len)
{
	if (NULL == des || NULL == src || des_len <= 1)
	{
		return;
	}
	...
	size_t copy_len = des_len - 1;

	memset(des, 0, des_len);
	strncpy(des, src, copy_len);
}
...
int main()
{
	...
	char buff[4];
	safe_strncpy(buff, "hello", sizeof(buff));
	printf("buff: %s\n", buff);

	safe_strncpy(buff, "h", sizeof(buff));
	printf("buff: %s\n", buff);
